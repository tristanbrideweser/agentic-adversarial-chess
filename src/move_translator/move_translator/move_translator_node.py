...
class MoveTranslatorNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._prev_fen: Optional[str] = None
    ...
    def _on_board_state(self, msg: String) -> None:
        ...
        """
        with self._lock:
            self._prev_fen = self._cached_fen
    ...
    def _on_apply_move(self, msg: String) -> None:
        ...
        with self._lock:
            prev = self._prev_fen
            curr = self._cached_fen

        pre_move_fen = chess.STARTING_FEN
        for candidate in [prev, curr, chess.STARTING_FEN]:
            if candidate is None:
                continue
            try:
                _b = chess.Board(candidate)
                _m = chess.Move.from_uci(uci)
                if _m in _b.legal_moves:
                    pre_move_fen = candidate
                    break
            except Exception:
                continue
        self.get_logger().info(f"Translating move '{uci}' from FEN: {pre_move_fen}")

        # If no FEN cached yet, assume starting position (first move of the game)
        if pre_move_fen is None:
            pre_move_fen = chess.STARTING_FEN
            self.get_logger().warn(
                f"No FEN cached for move '{uci}' — assuming starting position."
            )
            return
